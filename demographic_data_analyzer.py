...
def calculate_demographic_data(print_data=True):
    ...
    higher_education_rich = (df['education'].isin(['Bachelors', 'Masters', 'Doctorate']) & (df['salary']=='>50K')).sum() / df.shape[0] * 100
    lower_education_rich = ((~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])) & (df['salary']=='>50K')).sum() / df.shape[0] * 100
    ...
    rich_percentage = ((df['hours-per-week']==1) & (df['salary']=='>50K')).sum()/df.shape[0] * 100

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = df[df['salary']=='>50K'].groupby('native-country').size().idxmax()
    highest_earning_country_percentage = df[df['salary']=='>50K'].groupby('native-country').size().max()/df.shape[0] * 100
